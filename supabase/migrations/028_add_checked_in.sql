-- Migration 028: registra se o participante fez check-in no evento (presença física),
-- vindo da coluna "Fez check-in" do export bruto da plataforma de inscrições.
-- NULL = dado não disponível (import anterior a esta migration, ou planilha sem a coluna);
-- true/false = presença confirmada explicitamente pelo check-in do evento.

ALTER TABLE participants ADD COLUMN IF NOT EXISTS checked_in boolean;
  ...
  LOOP
    BEGIN
      ...
      INSERT INTO participants (
        ...
        registered_at, checked_in, import_job_id
      ) VALUES (
        p_edition_id,
        v_ticket_id,
        v_email,
        ...
        CASE
          WHEN v_row->>'checked_in' IN ('true','false')
            THEN (v_row->>'checked_in')::boolean
          ELSE NULL
        END,
        p_import_job_id
      )
